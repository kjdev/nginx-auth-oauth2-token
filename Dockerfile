...
./configure \
  ...
  --add-dynamic-module=..
make modules
cp objs/ngx_http_auth_oauth2_token_module.so /usr/lib/nginx/modules/
EOS
...
RUN --mount=type=cache,target=/var/cache/apk sh -ex <<'EOS'
apk add jansson
# load module: ngx_http_auth_oauth2_token_module.so
sed -i '/events {/i load_module "/usr/lib/nginx/modules/ngx_http_auth_oauth2_token_module.so";' /etc/nginx/nginx.conf
EOS

COPY --from=builder /usr/lib/nginx/modules/ngx_http_auth_oauth2_token_module.so /usr/lib/nginx/modules/ngx_http_auth_oauth2_token_module.so
