...
- Cap the IdP response size at 64 KiB with a module-defined constant
  - Use the module-defined `NGX_AUTH_OAUTH2_TOKEN_HTTP_RESPONSE_MAX_SIZE` (64 KiB) instead of `nxe-json`'s `NXE_JSON_MAX_SIZE` (1 MiB)
  - RFC 7662 / RFC 8693 responses are typically a few KiB, so a larger cap would leave an unnecessarily large memory amplification surface
